...
class Design_36_36_OnScreenButton extends ActorScript
{          	
...
nameMap.set("touching", "_touching");
_touching = false;
...
/* ======================== When Creating ========================= */
        ...
        _touching = false;
propertyChanged("_touching", _touching);
        #if mobile
            recycleActor(actor);
#end
...
if(wrapper.enabled)
{
        if((!(_touching) && actor.isMousePressed()))
{
            _touching = true;
propertyChanged("_touching", _touching);
